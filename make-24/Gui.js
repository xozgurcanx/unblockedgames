...
{
  if(this.rootScene.width != width || this.rootScene.height != height)
  {
  ...
  var x = width / 2;
  var y = height / 2;
  if(x != this.rootScene.x || y != this.rootScene.y)
  {
...
{
  if(!this.isNeedMouseOverOut) return;
  this.isMouseOver = true;

  this.emit('mouse_over', {target: this});
}
...
Gui.BaseButton.prototype.onMouseOutListener = function(event)
{
  ...
  this.emit('mouse_out', {target: this});
}

Gui.BaseButton.prototype.update = function()
{
  if(this.isMouseOver && this.isNeedMouseOverOut) 
  {
    // console.log('a', interaction.mouse);
    // console.log(this, this.isMouseOver);
    if(!this.checkMouseOver()) this.onMouseOutListener();
  }
}

Gui.BaseButton.prototype.checkMouseOver = function() 
{
  // if(!point || !container) return;

  var mouse = interaction.mouse.global;

  var globalTransform = Util.getGlobalTransform(this);

  if(mouse.x < globalTransform.x - globalTransform.width/2 || mouse.x > globalTransform.x + globalTransform.width/2) return false;
  if(mouse.y < globalTransform.y - globalTransform.height/2 || mouse.y > globalTransform.y + globalTransform.height/2) return false;

  // var globalScalePosition = 

  // var cp = container.getGlobalPosition();
  // console.log(cp, point, container.width, container.height);
  // if(point.x < cp.x-container.width/2 || point.x > cp.x + container.width/2) return false;
  // if(point.y < cp.y-container.height/2 || point.y > cp.y + container.height/2) return false;
...
Gui.createSimpleButton = function(config, data)
{
  ...
  if(data.pathToMouseOverBg)
  {
    var buttonOverBg = new PIXI.Sprite(assetsManager.getTexture('texture_atlas', data.pathToMouseOverBg));
    button.addChildAt(buttonOverBg, 0);
    buttonOverBg.anchor.set(0.5, 0.5);
    buttonOverBg.alpha = 0;

    button.on('mouse_over', function(e)
    {
      buttonOverBg.alpha = 1;
    }, button);
    button.on('mouse_out', function(e)
    {
      // var obj = interaction.hitTest(app.mouse);
      // console.log(obj);
      // obj.alpha -= 0.2;
      // if(button.hitTest(app.mouse)) return;
      buttonOverBg.alpha = 0;

      // console.log(button.hitTest);
    }, button);

    button.mouseOverBg = buttonOverBg;
  }
  if(data.pathToSkin)
  {
