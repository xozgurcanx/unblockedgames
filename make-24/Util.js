...
Util.permutationsWithRepetition = function(src, len){
...
Util.permutations = function(array, k)
{
 var m=[];
 var combinations = [];
 var indices = [];
...
Util.getGlobalTransform = function(obj)
{
  ...
  var p = new PIXI.Point(obj.x, obj.y);
  if(obj.parent != null)
  {
    p = obj.parent.toGlobal(p);
  }

  return { rotation: globalRotation, scaleX: globalScaleX, scaleY: globalScaleY, x: p.x, y: p.y, width: obj.width * globalScaleX, height: obj.height * globalScaleY };
}
...
Util.getMoveVector = function(speed, angle, position)
{
  angle = angle * Util.TO_RADIANS;

  var p = new PIXI.Point(speed * Math.cos(angle), speed * Math.sin(angle));
...
FBFAnimation.prototype = Object.create(PIXI.Container.prototype);
FBFAnimation.prototype.constructor = FBFAnimation;

FBFAnimation.prototype.addSequence = function(name, textureAtlas, path, nStart, nEnd, fps, data)
{
  var self = this;
...
FBFAnimation.prototype.switchSequence = function(name)
{
...
FBFAnimation.prototype.play = function()
{
  if(this.sequence == null) return;
  
  this.sequence.animation.play();
}
FBFAnimation.prototype.gotoAndPlay = function(frame)
{
  // this.sequence.currentFrame = frame-1;
  // this.play();
  this.sequence.animation.gotoAndPlay(frame-1);
}
FBFAnimation.prototype.stop = function()
{
  if(this.sequence == null) return;

  this.sequence.animation.stop();
}
FBFAnimation.prototype.gotoAndStop = function(frame)
{
  this.sequence.animation.gotoAndStop(frame-1);
}
FBFAnimation.prototype.onFrameChange = function(frame, sequence)
{
...
FBFAnimation.prototype.update = function()
{
  if(this.sequence == null) return;
