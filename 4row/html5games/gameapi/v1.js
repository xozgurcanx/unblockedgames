...
window.devgru_gameID = window.devgru_gameID || '';
window.devgru_gameJS = window.devgru_gameJS || [];
...
fg_api.prototype.adsModule = function() {
	...
	module.prototype.init = function() {
		self.config.ads = faZepto.extend({
			"off": (window.devgru_env === "vp" || self.config.gameParams.ad_type === "off"),
			"show_initial": false,
			"show_video": false
		}, self.config.ads);
		...
		switch (M.provider) {
			...
			case 'dfp':
				...
				faZepto.getScript("https://imasdk.googleapis.com/js/sdkloader/ima3.js",
					...
					function() {
						sdkLoadedCallback();
						self.tracking.trackEvent("AdBlocker event", "DFP", devgru_gameID);
					});
	...
	module.prototype.rewardedAd = function(callback) {
		self.tracking.trackEvent('Ad Event', 'Rewarded Ad', devgru_gameID);
	...
	module.prototype.getAdTagUrl = function() {
		...
		myadTagUrl =
			"https://securepubads.g.doubleclick.net/gampad/ads?impl=s&gdfp_req=1&env=vp&output=vast&overlay=0&unviewed_position_start=1&url=[referrer_url]&correlator=[timestamp]&ciu_szs=";
	...
	module.prototype.showAdModal = function(options, contentDiv) {
		var opts = faZepto.extend({
			...
			faketime: 3,
			overlayStyle: {
				"background": 'url("' + self.config.blurred_thumb + '") no-repeat center center',
	...
	module.prototype.showHouseAd = function(callback, force) {
		...
		// show house ad
		var url = '/sda/housead/' + window.devgru_gameID + '/' + window.devgru.config
			.aid;
	...
	module.prototype.showMatchedContent = function(callback, force) {
		...
		// show house ad
		var url = '/sda/matched_content/' + window.devgru_gameID + '/' + window.devgru.config
			.aid;
		self.iframe.show(url, opts);
		self.modal.setHeader(self.translate("api.ad_modal_header2"));
		self.modal.setCloseCallback(opts.returnCallback);
	...
	module.prototype.showBlocker = function(callback, force) {
		...
		// show house ad
		var url = '/sda/blocker/' + window.devgru_gameID + '/' + window.devgru.config
			.aid;
		self.iframe.show(url, opts);
	...
	module.prototype.showTeaser = function(callback) {
		...
		// show banner
		var url = '/sda/teaser/' + window.devgru_gameID + '/' + window.devgru.config
			.aid;
		self.iframe.show(url, opts);
	...
	module.prototype.showStickyAd = function(callback, force) {
		if (faZepto('#SP_sticky_mobile_bottom').length <= 0) {
			faZepto.getScript("//surgeprice.com/display/async/defKYzzSGzk88nn2S/devgru.com/ariel.js",
				function() {
					faZepto(self.bodyElement).prepend('<div id="devgru.com_sticky_mobile_bottom_ingame" class="surgeprice"><script data-cfasync="false" type="text/javascript">surgeprice.display("devgru.com_sticky_mobile_bottom_ingame");</script></div>');
...
fg_api.prototype.notifyModule = function() {
	...
	module.prototype.init = function() {
		...
		if (window.devgru_env === "vp" || detection.is.pc) {
...
fg_api.prototype.adaptersModule = function() {
	...
	adaptersPrototype.init = function() {
		var section = "",
			subsection = "";
		if (typeof devgru_adapters !== "undefined") {
			for (section in devgru_adapters) {
				//self.log(section);
				for (subsection in devgru_adapters[section]) {
					//self.log(subsection);
					this.add(section, subsection, devgru_adapters[section][subsection]);
				}
			}
		}
	};
...
fg_api.prototype.getMoreGamesButtonImage = function (forceAbsolute) {
	...
	if (forceAbsolute || (
		window.location.hostname.indexOf('cdn.devgru.com') === -1 && 
		window.location.hostname.indexOf('dev.devgru.com') === -1)
		) {
...
fg_api.prototype.sdkModule = function() {
	...
	module.prototype.init = function() {
		...
		// check if devgru sdk is loaded
		if(window.top != window.self && !hasLocalStorageKeys()) {
			window.top.postMessage({
				scope: 'devgru_sdk', 
				method: 'getData', 
				time: +(new Date), 
				package_id: window.devgru_gameID, 
				aid: self.config.aid
			}, "*");
		}
	}
		...
		return {
			...
			clear: function () {
				for (var key in window[storageType]) {
					if(key.indexOf(window.devgru_gameID) === 0) this.removeItem(key, "");
				}
			},
			key: function (i) {
				var keys = [];
				for (var key in window[storageType]) {
					if(key.indexOf(window.devgru_gameID) === 0) keys.push(key);
				}
...
fg_api.prototype.highscoresModule = function() {
	...
	highscoresPrototype.submit = function(level, score) {
		...
		self.tracking.trackEvent('Highscore event', 'submit', devgru_gameID + ':level:' + level);
		...
		self.showAd(function() {
			...
			// Save local
			self.sessionStorage.setMaxItem("devgru:level", level);
			if (self.sessionStorage.setMaxItem("devgru:score", score)) {
				doSubmit(score);
			}
	...
	highscoresPrototype.get = function(){
		return self.sessionStorage.getItem("devgru:score");
	};

	highscoresPrototype.del = function(){
		self.sessionStorage.delItem("devgru:score");
	};
...
fg_api.prototype.screenshotModule = function() {
	...
	function discardScreenshotUpload(event) {
		self.tracking.trackEvent('Screenshot event', 'discard', devgru_gameID);
		self.modal.close();
	}
	...
	function uploadScreenshot(imageSrc) {
		...
		self.tracking.trackEvent('Screenshot event', 'save', devgru_gameID);
	...
	function handleScreenshotSave(data){
		...
		function imageLoaded(imageHasLoaded) {
			self.tracking.trackEvent('Screenshot event', 'uploaded:' + (imageHasLoaded ? '1':'0'), devgru_gameID);
	...
	function takeScreenshot(area) {
		...
		M.txid = null;
		self.tracking.trackEvent('Screenshot event', 'shoot', devgru_gameID);
...
fg_api.prototype.navigationModule = function() {
	...
	navigationPrototype.renderRelatedGames = function(choices) {
		...
		for(var i = 0; i < choices.length; i++) {
			...
			faZepto('.fg-related-games').append('<li><a href="https://play.devgru.com/'+packageId+'/'+self.config.aid+'"><img src="https://img.cdn.devgru.com/portal/html5games/images/tmp/60/'+thumb+'" alt=""></li>');
		}
	};
	...
	navigationPrototype.bindEvents = function() {
		...
		faZepto("[data-devgru-href]").css("cursor", "pointer").each(function() {
			var callback = function() {
				var link = faZepto(this).attr("data-devgru-href");
		...
		faZepto("[data-devgru-fullscreen]").each(function() {
			self.handleClick(this, M.switchFullscreen);
		});

		faZepto("[data-devgru-screenshot]").each(function() {
...
fg_api.prototype.getAppLink = function() {
	return 'https://play.devgru.com/' + window.devgru_gameID + '.app/' + this.config.aid;
};
...
fg_api.prototype.orientationModule = function() {
	...
	M.init().onorientationchange(function() {
		self.log('devgru.onorientationchange: ', self.getOrientation(), window.innerWidth, window.innerHeight);
	});
...
fg_api.prototype.onOrientationChange = fg_api.prototype.onorientationchange = function(callback) {
	if (typeof callback !== 'function') {
		this.error('devgru.onorientationchange: no valid callback provided.');
	}
...
fg_api.prototype.trackingModule = function() {
	...
	trackingPrototype.init = function() {
		devgru_dataLayer = window.devgru_dataLayer || window.dataLayer || [];

		// Use dataLayer variables
		devgru_dataLayer.push({
			'packageId': window.devgru_gameID,
	...
	trackingPrototype.data = function(dataObject) {
		devgru_dataLayer.push(dataObject);
	};
	...
	trackingPrototype.trackEvent = function(category, action, label, value, nonInteraction) {
		...
		if (typeof M.floodProtectionMap[key] !== "undefined" && 
			...
			// M.trackEvent(M.floodProtectionPrefix, key, window.devgru_gameID);
...
!function(a, b) {
	var devgru_analytics = {
			...
			return new Promise(function(resolve, reject) {
				...
				var updateKey = function(key, value) {
					if (typeof key === 'string' && key.length) {
						if (key in devgru_analytics.stats && 
							(typeof value === "undefined" || value === null)) {
							delete devgru_analytics.stats[key];
						} else {
							if(typeof value !== "undefined" && value) {
								devgru_analytics.stats[key] = value;
							}
						}
				...
				if (devgru.hasOwnProperty("adapters")) {
					devgru.adapters.run("analytics", "trackStats", key, value);
				}
		...
		'getStats': function() {
			...
			return new Promise(function(resolve, reject) {
				resolve(devgru_analytics.stats);
			});
		},
		...
		'trackEvent': function(event, params) {
			return new Promise(function(resolve, reject) {
				if (typeof devgru_analytics[event] === "undefined") {
				...
				devgru_analytics.events.push({event: event, params: params});

				if (devgru.hasOwnProperty("adapters")) {
					devgru.adapters.run("analytics", "trackEvent", event, params);
				}

				if (devgru_analytics.TRIGGER_AD.indexOf(event) > -1) {
					return devgru.showAd(function() {
						resolve(event, params);
					});
				}

				if (devgru_analytics.TRIGGER_LEVELSCORE.indexOf(event) > -1) {
					devgru.submitHighscore(params['levelName'], params['levelScore']);
					return resolve(event, params);
				}

				if (devgru_analytics.TRIGGER_TOTALSCORE.indexOf(event) > -1) {
					devgru.submitHighscore('TOTAL', params['totalScore']);
					return resolve(event, params);
				}
		...
		'getEvents': function() {
			// for debugging purposes
			return new Promise(function(resolve, reject) {
				resolve(devgru_analytics.events);
			});
		},
		...
		'trackScreen': function(screen, pageTitle) {
			return new Promise(function(resolve, reject) {
				...
				if (typeof devgru_analytics[screen] === "undefined") {
				...
				devgru_analytics.currentScreen = screen;
				devgru_analytics.pageTitle = pageTitle;

				if (devgru.hasOwnProperty("adapters")) {
					devgru.adapters.run("analytics", "trackScreen", screen, pageTitle);
				}
		...
		'getScreen': function() {
			return devgru_analytics.currentScreen;
		},
		'getPageTitle': function() {
			return devgru_analytics.pageTitle;
		},
	...
	// export devgru_analytics object to global object
	b[a] = devgru_analytics;

}('devgru_analytics', window);
...
fg_api.prototype.firebaseModule = function() {
	...
	module.prototype.initializeFirebase = function() {
		...
		M.promise = new Promise(function(resolve, reject) {
			...
			faZepto.getScript('https://www.gstatic.com/firebasejs/3.8.0/firebase.js', function() {
				// Initialize Firebase
				var config = {
					apiKey: "AIzaSyAN2Zj_J4pObQIbXxYYDYatv-VkMvxLoVQ",
					authDomain: "devgru-01.firebaseapp.com",
					databaseURL: "https://devgru-01.firebaseio.com",
					storageBucket: "devgru-01.appspot.com",
				};
...
fg_api.prototype.lsgModule = function() {
	...
	module.prototype.init = function() {
		...
		this.is_active   = !(window.devgru_env === "vp");
	...
	module.prototype.pickDynamicRule = function() {
		return new Promise(function(resolve, reject) {
			setTimeout(reject, 4E3);
			self.firebase.isReady().then(function() {
				M.getRules().then(function() {
					var nbRules = 5;
					var amountThreshold = 25; // Percent
					var amountMinCoverage = 25; // Percent
					...
					Promise.all([
						...
						if (rulesMap) {
							...
							faZepto.each(rulesMap, function(key, rule) {
								...
								var fac = (100 - Math.round(fill_rate * 100));
								for (var i = 0; i < fac * fac; i++) {
...
fg_api.prototype.gameModule = function() {
	...
	gamePrototype.prepare = function() {
		...
		return new Promise(function (resolve, reject) {
			(function next() {
				if (!window.devgru_pregameJS || !window.devgru_pregameJS.length) {
					resolve(M);
					return;
				}
				var currentScript = window.devgru_pregameJS.shift();
	...
	gamePrototype.init = function() {
		...
		(function next() {
			...
			if (!window.devgru_gameJS.length) {
				faZepto(self.rootElement).trigger('faGame:loaded');
				self.spinner.hide();

				if (self.config.package_id.indexOf('disney-') === 0) {
					var url = "http://eiskoenigin.disney.de/gewinnspiel/?ex_cmp=dis_de:franchises:fro:devgru:0417";
					...
					self.geo.getCountryCode().then(function(locale) {
						...
						if (['DE', 'AT', 'CH'].indexOf(locale) > -1) {
							...
							self.modal.setContent(faZepto('<div style="color: #fff; background:transparent;text-align:center"><a href="' + url + '" target="_blank" style="display:block;cursor:pointer;"><img src="//img.cdn.devgru.com/marketing/partner/disney/frozen-ostern_300x250.jpg" alt="" width="300" height="250"></a></div>').get(0));
			...
			var currentScript = window.devgru_gameJS.shift();
	...
	gamePrototype.pause = function() {
		...
		try {
			if (typeof window.devgru_onPauseRequested == 'function') {
				window.devgru_onPauseRequested();
				return true;
			}
			// Phaser
	...
	gamePrototype.resume = function() {
		...
		try {
			if (typeof window.devgru_onResumeRequested == 'function') {
				window.devgru_onResumeRequested();
				return true;
			}
			// Phaser
			if (window.game && typeof window.game.paused !== "undefined") {
...
fg_api.prototype.fullscreenModule = function() {
	...
	fullscreenPrototype.init = function() {
		...
		if (!this.isSupported) {
			faZepto("[data-devgru-fullscreen]").remove();
		}
	};
...
fg_api.prototype.flashModule = function() {
	...
	function module() { // define private vars
		...
		// swfobject
		this.swfUrl 		= window.devgru_gameID+".swf";
		...
		this.flashvars 		= {
								fg_aid : window.devgru.config.aid,
								fg_uid : window.devgru.config.uuid,
								fg_game : window.devgru_gameID
							};
		this.params 		= {
...
fg_api.prototype.playerModule = function() {
	...
	function module() {
		this.playerUrl = 'https://player-api.devgru.com';

		if (location.search.indexOf('.staging.gc.') >= 0) {
			this.playerUrl = 'https://player-api.staging.gc.devgru.com';
		}
...
fg_api.prototype.socketModule = function() {
	...
	function module() {
		this.httpsUrl = 'https://ws-api.devgru.com/sock';
		this.wsUrl = 'wss://ws-api.devgru.com/ws';

		if (location.search.indexOf('.staging.gc.') >= 0) {
			this.httpsUrl = 'https://ws-api.staging.gc.devgru.com/sock';
			this.wsUrl = 'wss://ws-api.staging.gc.devgru.com/ws';
		}
		else if (location.hostname.indexOf('dev.') >= 0) {
			this.httpsUrl = "https://ws-api.staging.gc.devgru.com/sock";
			this.wsUrl = "wss://ws-api.staging.gc.devgru.com/ws";
...
fg_api.prototype.payModule = function() {
	...
	module.prototype.init = function() {
		...
		this.paymentUrl = 'https://commune.devgru.com/product.html';

		if (location.search.indexOf('.staging.gc.') >= 0) {
			this.paymentUrl = 'https://commune.staging.gc.devgru.com/product.html';
		}
		else if (location.hostname.indexOf('dev.') >= 0) {
			this.paymentUrl = "https://commune.dev.devgru.com/product.html";
		}

		this.paymentApiUrl = 'https://payment-api.devgru.com';

		if (location.search.indexOf('.staging.gc.') >= 0) {
			this.paymentApiUrl = 'https://payment-api.staging.gc.devgru.com';
		}
		else if (location.hostname.indexOf('dev.') >= 0) {
			this.paymentApiUrl = "https://payment-api.staging.gc.devgru.com";
		}
	...
	module.prototype.selectMethod = function(method) {
		self.tracking.trackEvent('Payment event', 'Select method ' + method, window.devgru_gameID);
	...
	module.prototype.trackError = function(message) {
		self.tracking.trackEvent('Payment event', 'Error: ' + message, window.devgru_gameID);
	};

	module.prototype.trackCountry = function(code) {
		self.tracking.trackEvent('Payment event', 'Country Code: ' + code, window.devgru_gameID);
	};
	...
	module.prototype.getItems = function() {
		...
		self.log("devgru.pay items: ", paymentItems);
		return paymentItems;
	};
	...
	module.prototype.beginTransaction = function(itemId, paymentSystem) {
		...
		self.tracking.trackEvent('Payment event', 'Select item ' + itemId, window.devgru_gameID);
	...
	module.prototype.buy = function(itemId, paymentSystem) {
		...
		websocketApi.getPromise()
			...
			.then(function (url){
				...
				// 	}
				// );
				// devgru.modal.activateCloseOverlay(false);
				// setTimeout(function() {
				// 	self.modal.showCloseBtn();
				// }, 10E3);
			});
	...
	module.prototype.showSelection = function(itemId, options) {
		...
		self.tracking.trackEvent('Payment event', trackingText, window.devgru_gameID);
		self.spinner.show(); 
		self.modal.create(opts);
		self.modal.activateCloseOverlay(false);
	...
	module.prototype.fireSuccessCallback = function(itemId) {
		...
		self.tracking.trackEvent('Payment event', 'Bought item ' + itemId, window.devgru_gameID);

		self.tracking.trackEvent('Payment event', 'Payment success', itemId + ' in ' + window.devgru_gameID, Number(item.price));
...
fg_api.prototype.debugModule = function() {
	...
	module.prototype.init = function() {
		if (self.debug && self.config.aid !== 'A-CDTEST') {
		...
		self.adapters.add('analytics', 'trackStats', function(key, value) {
			devgru.log('%c trackStats ', 'background: #333333; color: white; display: block;');
			if(typeof key === "string") {
				devgru.log(key + ": " + value);
			} else {
				devgru.log(key);
			}
		});
		...
		self.adapters.add('analytics', 'trackEvent', function(event, params) {
			devgru.log('%c '+event+" %c (trackEvent)", 'background: #0092c3; color: white; display: block;', '');
		 	devgru.log(params);
		});

		self.adapters.add('analytics', 'trackScreen', function(screen, pageTitle){
			devgru.log('%c '+screen+' %c (trackScreen)'+(pageTitle ? ' - pageTitle: '+pageTitle : ''), 'background: #f08119; color: white; display: block;', '') ;
		});
...
fg_api.prototype.videoModule = function() {
	...
	module.prototype.init = function() {
		...
		M.player.src([
			{
				type: "video/mp4",
				src: "https://assets.cdn.devgru.com/marketing/videos/" + self.config.video.sources.mp4
			}
		]);
		...
		M.player.brand({
			image: 'devgruFLogo.svg',
			title: 'Proudly brought to you by devgru.com',
			destination: 'https://devgru.com',
			destinationTarget: '_blank'
		});

		M.player.watermark({
			image: 'devgruFWhiteLogo.svg',
			position: 'top-right',
			fadeTime: 1000,
			url: 'https://devgru.com'
		});
...
fg_api.prototype.init = function() {
	...
	function onloadCallback(){
		...
		if (detection.is.pc && window.console) {
			console.log("Developed by devgru");
		}
	}
